...
export const serviceManagementStatusAddonName = 'dosai-service-management-status.node';
export const serviceManagementStatusAddonIdentifier =
...
export async function buildServiceManagementStatusAddon(outputPath) {
  const absoluteOutput = resolve(outputPath);
  const outputDirectory = dirname(absoluteOutput);
  const moduleCachePath = resolve(outputDirectory, '.status-addon-module-cache');
  ...
  try {
    ...
    if (!(await stat(absoluteOutput)).isFile()) {
      throw new Error('DOSAI_STATUS_ADDON_BUILD_0001');
    }
    await execFileAsync(
      '/usr/bin/codesign',
      [
